...
for (i in 37:end){
    ...
    temporary_r <- ts(temporary_r, start = c(2010,1), frequency = 4)
    ...
    forecast_mortgage <- ts(forecast_mortgage$mean, start = c(2010,1), frequency = 4)
    #forecast the i-th observation
    fore <- forecast(fit4, xreg = forecast_mortgage,h = 12)
    fore <- fore$mean
    print <- ts(fore, start = c(end_year, end_month + 1 ), frequency = 4)
    lines(print, col="red")
}
